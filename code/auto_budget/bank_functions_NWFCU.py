...
def onehot_encode_words(content):
    ...
    content = content.apply(remove_stopwords)
...
def encode_data(df):
    ...
    df[2] = 0
    df[3] = 0
    words_df = onehot_encode_words(df['Content'])
    df = df.join(words_df)
    return df
...
def self_categorization(df, ref):
    ...
    df['Cat_Self_Pred'] = categorize(df)
